...
void cfg_file_parse(u8 idx)
{
	...
	//-----------------------------CFG_KEY_DOUBLE_CFG_ID----------------------------//
	//double_cfg_t double_cfg_tmp;
	ret = syscfg_read(CFG_KEY_DOUBLE_CFG_ID, &double_cfg, sizeof(double_cfg_t));
	if (ret < 0) {
		log_debug("read double_cfg error");
		double_cfg.spatialMode = 0;
		double_cfg.codecState = 0;
		double_cfg.toneVolumeLevel =D_DEFULAT_TONE_VOL_LEVEL;
	}
	
    if(double_cfg.toneVolumeLevel ==0)
       double_cfg.toneVolumeLevel =D_DEFULAT_TONE_VOL_LEVEL;

	log_info("Joe:spatialMode is:%d", double_cfg.spatialMode);
	log_info("Joe:codecState is:%d", double_cfg.codecState);
	log_info("Joe:toneVolumeLevel is:%d", double_cfg.toneVolumeLevel);
