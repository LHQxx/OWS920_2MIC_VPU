...
//add by joe
BT_USER_COMM_VAR mmi_var;
double_cfg_t   double_cfg;
...
void app_var_init(void)
{
	...
	//add by joe, init user variable  value
#if MMI_CC_INIT_VARIABLE
	mmi_var.a2dp_stream_ing =FALSE;
	mmi_var.poweron_recon_timeout =FALSE;
	mmi_var.linkloss_recon_timeout =FALSE;
	mmi_var.first_paired =FALSE;
	mmi_var.bt_discover_connectable =BT_STA_OFF;
	mmi_var.page_type =PAGE_TYPE_NULL;

	log_info("Joe: Building_month-date-year: %s \n", __DATE__);
	log_info("Joe: Building_hour-minute-second: %s \n", __TIME__);
#endif

#if MMI_CC_SHIPPING_MODE
	gpio_set_mode(PORTB, PORT_PIN_0, PORT_OUTPUT_LOW);
	BOAT_PIN_OUT_LEVEL(FALSE);
#endif
}
...
void check_power_on_key(void)
{
    ...
    while (1) {
        ...
        if (get_power_on_status()) {
            ...
            delay_10ms_cnt++;
            key_ng_cnt = 0;
#if MMI_CC_LONGPRESS_KEY_xS_POWER_ON
            if (delay_10ms_cnt > D_TIMER_CNT) {
#else
