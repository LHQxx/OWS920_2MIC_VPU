...
#if MMI_CC_SPATIAL_MODLE
#include "spatial_effects_process.h"
#define TWS_FUNC_ID_SPATIAL_EFFECT_SWITCH_SYNC    TWS_FUNC_ID('S', 'E', 'S', 'S')
static void bt_tws_spatial_effect_switch_sync(int priv, int err)
{
    printf("Joe:bt_tws_spatial_effect_switch_sync\n"); 
    ...
    double_cfg.spatialMode =mode;
	syscfg_write(CFG_KEY_DOUBLE_CFG_ID, &(double_cfg.spatialMode), 1);
	printf("Joe:Current spatial mode is:%d\n", mode);

}

TWS_SYNC_CALL_REGISTER(app_spatial_effect_switch_sync) = {
	.uuid = TWS_FUNC_ID_SPATIAL_EFFECT_SWITCH_SYNC,
	.task_name = "app_core",
	.func = bt_tws_spatial_effect_switch_sync,
};

/**
 * @brief TWS同步空间音效切换
 */
void bt_tws_sync_spatial_effect_switch(void)
{
    ...
    int err = tws_api_sync_call_by_uuid(TWS_FUNC_ID_SPATIAL_EFFECT_SWITCH_SYNC, 0, 400);
    if (err < 0) {
        bt_tws_spatial_effect_switch_sync(0,0);
    }
