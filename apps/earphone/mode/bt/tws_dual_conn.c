...
//MMI_CC_PAIR_TIMEOUT_INTO_CONECTABLE
...
#if MMI_CC_DISABLE_PAIR_LINKLOSS_AFTER
	//LINKLOSS中和LINKLOSS 超时后，都不能进入PAIR MODE
	if(((mmi_var.page_type ==PAGE_TYPE_LINKLOSS)||(mmi_var.linkloss_recon_timeout==TRUE))&&(scan_enable ==1))
	{
		scan_enable = 0;
		printf("Joe:Device_linkloss_disable_enter_pair ");
	}
#endif
...
#if MMI_CC_ENTER_PAIR_BEFORE_ADD_DET
	//if(mmi_var.bt_discover_connectable !=((conn_enable << 1) | scan_enable))
	{
	   mmi_var.bt_discover_connectable =((scan_enable << 1) | conn_enable);
	   printf("Joe:bt_set_status is %d\n", mmi_var.bt_discover_connectable);
	   //app_send_message(APP_MSG_MIN_VOL, 0);
	}
#endif

#if MMI_CC_PAIR_TIMEOUT_INTO_CONECTABLE
	if ((scan_enable || conn_enable)) {
		int connect_device = bt_get_total_connect_dev();
		if (page_list_empty()) {
			//Only into pair mode,can be send"APP_MSG_BT_IN_PAIRING_MODE" msg
			if(scan_enable ==1)
			{
				app_send_message(APP_MSG_BT_IN_PAIRING_MODE, connect_device);
				if(g_dual_conn.inquiry_scan_time)
				{
				   sys_timeout_del(g_dual_conn.inquiry_scan_time);
				...
				printf("Joe:Update_pair_timeout_timer ");
		   ...
		   //超距回连/回连超时后需要进入
		   if(mmi_var.page_type !=PAGE_TYPE_POWERON)
			   app_send_message(APP_MSG_BT_IN_PAGE_MODE, 0);			 
		}
	}
...
static void close_inquiry_scan(void *p)
{
	...
	else
	{
	    if(bt_get_total_connect_dev() ==0)
	    {
			puts("Joe:Pair_timeout_into_conectable\n");
...
static int dual_conn_try_open_inquiry_page_scan()
{
    ...
    if (connect_device == 0) {
#if MMI_CC_ENTER_PAIR_BEFORE_ADD_DET
		if(mmi_var.bt_discover_connectable !=0x03)
		   write_scan_conn_enable(1, 1);
#else
		write_scan_conn_enable(1, 1);
#endif
...
#if MMI_CC_PAIR_TIMEOUT_INTO_CONECTABLE
	if(timeout ==TCFG_BT_POWERON_PAGE_TIME * 1000)
	{
		mmi_var.page_type =PAGE_TYPE_POWERON;
	}
	else if(timeout ==TCFG_BT_TIMEOUT_PAGE_TIME * 1000)
	{
		mmi_var.page_type =PAGE_TYPE_LINKLOSS;
	}
	else
	{
		mmi_var.page_type =PAGE_TYPE_NULL;
	}
#endif
...
#if MMI_CC_PAIR_TIMEOUT_INTO_CONECTABLE
	mmi_var.page_type =PAGE_TYPE_NULL;
	printf("Joe:page_type_reset\n");
#endif
#if MMI_CC_DISABLE_PAIR_LINKLOSS_AFTER
	mmi_var.linkloss_recon_timeout =FALSE;
#endif
...
//回连超时的时间=单次回连时间（8S）+TWS的单次连接超时（6S）
//其中8S的时间内是不可发现和不可连接的，6S的时间内是“可以发现和可以连接”的
//8S+6S,依次LOOP,直到“开机回连超时（60S）”	溢出才结束，最后一直进入“可以发现和可以连接”

//linkloss发生后，也是执行以上的回连流程
//问题1：linkloss后会有间隔6S的可被发现的情况
//问题2：linkloss的回连超时时间，应该与无连接关机时间保持一致，否则linkloss timeout又会一样进入配对
    ...
    list_for_each_entry(info, &g_dual_conn.page_head, entry) {
        if (info == p) {
            ...
            if (time_after(jiffies, info->timeout)) {
                free(info);
                //add by joe
                printf("Joe: Powero_or_linkloss_reconnection_timeout:");
#if MMI_CC_PAIR_TIMEOUT_INTO_CONECTABLE
                if(mmi_var.page_type ==PAGE_TYPE_POWERON)
                {
                   mmi_var.poweron_recon_timeout =TRUE;
                }
                else if(mmi_var.page_type ==PAGE_TYPE_LINKLOSS)
                {
                   mmi_var.linkloss_recon_timeout =TRUE;
                }
...
#if MMI_CC_LINKKEY_MISSING_CANCEL_RECON
	//add by joe,"event->value == ERROR_CODE_PIN_OR_KEY_MISSING"
	//当BT回连时检测到设备端已经删除了LINKKEY,在回连时会触发一次“ERROR_CODE_PIN_OR_KEY_MISSING”
	extern const int config_delete_link_key;
	if((event->value ==ERROR_CODE_PIN_OR_KEY_MISSING)&&(config_delete_link_key))
	{
		clr_device_in_page_list();
		bt_cmd_prepare(USER_CTRL_PAGE_CANCEL, 0, NULL);
	}
...
static void tws_pair_timeout(void *p)
{
    ...
    if (!page_list_empty()) {
        dual_conn_page_device();
		//系统默认回连与配对是同步交替进行，但对外表现为回连（可连接），此处强行转换为pairing msg
