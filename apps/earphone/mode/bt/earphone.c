...
#if MMI_CC_CLEAR_ALL_DEVICE_RECORD
#define TWS_FUNC_ID_CLEAR_DEVICE_RECORD_SYNC    TWS_FUNC_ID('C', 'D', 'R', 'S')
static void set_clear_device_record(int priv, int err)
{
    printf("Joe:APP_MSG_DEL_ALL_REMOTE_DEV\n"); //不会删除掉TWS间的记录
	...
	#if MMI_CC_CPU_RESET_REBOOT_CLEAR_RECORD
	sys_enter_soft_poweroff(POWEROFF_RESET);
	#else
	sys_enter_soft_poweroff(POWEROFF_NORMAL_TWS);  //系统直接关机
	#endif
	
}

TWS_SYNC_CALL_REGISTER(app_clear_device_record_sync) = {
	.uuid = TWS_FUNC_ID_CLEAR_DEVICE_RECORD_SYNC,
	.task_name = "app_core",
	.func = set_clear_device_record,
};

/**
 * @brief TWS同步清除设备的记录
 */
static void tws_sync_clear_device_record(void)
{
    int err = tws_api_sync_call_by_uuid(TWS_FUNC_ID_CLEAR_DEVICE_RECORD_SYNC, 0, 400);
    if (err < 0) {
        set_clear_device_record(0,0);
    }
...
#if MMI_CC_GET_REMOTE_NAME_AFTER_CONNECTED
//Notes:
// CONFIG_LMP_NAME_REQ_ENABLE ==1
void user_get_remote_name(void)
{
	//u8 *addr = bt_get_current_remote_addr();
	
	//bt_cmd_prepare(USER_CTRL_READ_REMOTE_NAME, 0, NULL);
	//bt_cmd_prepare_for_addr(bt_addr, USER_CTRL_READ_REMOTE_NAME, 6, bt_addr); 
	//break;
	
	u8 *bt_addr =NULL;
	void *devices[1] ={NULL};
	int num =btstack_get_conn_devices(devices, 1);
	bt_addr =btstack_get_device_mac_addr(devices[0]);
	put_buf(bt_addr,6);
...
#if MMI_CC_BT_SUPPORT_LDAC
    if(double_cfg.codecState)
      bt_set_support_ldac_flag(0);
    else
...
#if MMI_CC_ADD_PRINTF_STATE_TIMER
/*----------------------------------------------------------------------------*/
/**@brief  设定一个MS级的定时器
   @param
   @return
...
*/
/*----------------------------------------------------------------------------*/
void state_check(void *priv)
{
   //printf("Joe: state check start\n");

   mmi_var.tmr5ms_cnt++;

   //50ms timer process
   if((mmi_var.tmr5ms_cnt%10) ==0)
   {
     ;
   }

   //100ms timer process
   if((mmi_var.tmr5ms_cnt%20) ==0)
   {
     ;
   }

   //500ms timer process
   if((mmi_var.tmr5ms_cnt%100) ==0)
   {
     ;
   }

   //1S timer process
   if((mmi_var.tmr5ms_cnt%200) ==0)
   {
#if 0
	   if(bt_get_connect_status()==BT_STATUS_INITING)
		  printf("Joe: BT_STATUS_INITING\n");
	   else if(bt_get_connect_status()==BT_STATUS_WAITINT_CONN)
		  printf("Joe: BT_STATUS_WAITINT_CONN\n");
	   else if(bt_get_connect_status()==BT_STATUS_AUTO_CONNECTINT)
		  printf("Joe: BT_STATUS_AUTO_CONNECTINT\n");
	   else if(bt_get_connect_status()==BT_STATUS_CONNECTING)
		  printf("Joe: BT_STATUS_CONNECTING\n");
	   else if(bt_get_connect_status()==BT_STATUS_TAKEING_PHONE)
		  printf("Joe: BT_STATUS_TAKEING_PHONE\n");
	   else if(bt_get_connect_status()==BT_STATUS_PLAYING_MUSIC)
		  printf("Joe: BT_STATUS_PLAYING_MUSIC\n");
	   else
		  printf("Joe: BT_STATUS_Unknow\n");
#endif

#if 0
       //bool page_list;
       //page_list =page_list_empty();
       if(tws_api_get_role()==TWS_ROLE_MASTER)
       {
		   //log_info("Joe: tws_state is 0x%x\n", tws_api_get_tws_state());
		   log_info("Joe: tws_state_is 0x%x\n", tws_host_get_local_role());
		   log_info("Joe: total_connect_dev %d\n", bt_get_total_connect_dev());
		   //log_info("Joe: page_list_empty %d\n", page_list);
		   log_info("Joe: get_bt_status %d\n", mmi_var.bt_discover_connectable);

       }

       if(page_list_empty())
       {
           log_info("Joe: non_reconnection\n");
       }
       else
       {
           log_info("Joe: reconnection_ing\n");
       }
#endif
   ...
   //5S timer process
   if((mmi_var.tmr5ms_cnt%1000) ==0)
   {
	    mmi_var.tmr5ms_cnt =0;
#if 0
		u8 remote_addr[6];
		u8 common_addr[6];
		u8 local_addr[6];
		...
		log_info("tws_remote_addr_mac:");
		put_buf(remote_addr, sizeof(remote_addr));
		
	    syscfg_read(CFG_TWS_COMMON_ADDR, common_addr, sizeof(common_addr));
		log_info("tws_common_addr_mac:");
		put_buf(common_addr, sizeof(common_addr));
		
	    bt_get_tws_local_addr(local_addr);
		log_info("tws_local_addr_mac:");
		put_buf(local_addr, sizeof(local_addr));

	    syscfg_read(CFG_TWS_LOCAL_ADDR, local_addr, sizeof(local_addr));
		log_info("tws_local_addr_mac22:");
		put_buf(local_addr, sizeof(local_addr));

		syscfg_read(CFG_BT_MAC_ADDR, local_addr, sizeof(local_addr));
		log_info("bt_mac:");
		put_buf(local_addr, sizeof(local_addr));
#endif

#if MMI_CC_PRINT_ROLE_xTIMER
		if(tws_api_get_role()==TWS_ROLE_MASTER)
		{
		   log_info("Joe: Current_role_is_Master\n");
		   //b_printf();
		   //mem_stats(); 

		   //log_info("Joe: get_num_of_remote_device_recorded %d\n", btstack_get_num_of_remote_device_recorded());
		}
		else if(tws_api_get_role()==TWS_ROLE_SLAVE)
		{
		   log_info("Joe: Current_role_is_Slave\n");
		}
		else
		   log_info("Joe: Current_role_is_Unknow\n");
	   }
#endif
}
#endif
...
#if MMI_CC_ADD_PRINTF_STATE_TIMER
	if(mmi_var.printf_check_timer ==0)
	{
	  mmi_var.printf_check_timer =sys_timer_add(NULL, state_check, STATE_CHECK_TIME);
	  printf("Joe: create_printf_check_timer");
	}
#endif
...
#if MMI_CC_ADD_PRINTF_STATE_TIMER
	if(mmi_var.printf_check_timer)
	{
	  sys_timer_del(mmi_var.printf_check_timer);
	  mmi_var.printf_check_timer =0;

	  printf("Joe: delete_printf_check_timer");
	}
#endif
...
int bt_app_msg_handler(int *msg)
{
    ...
    case APP_MSG_EARTCH_OUT_EAR:
...
#if MMI_CC_PLAY_PAIR_VP_TONE
		case APP_MSG_BT_IN_PAIRING_MODE:
		printf("Joe:tone_APP_MSG_BT_IN_PAIRING_MODE\n");
#if TCFG_USER_TWS_ENABLE
		...
		//回连超时，只有一次事件，用完后要清状态
		if(mmi_var.poweron_recon_timeout ==TRUE)
		{
		   mmi_var.poweron_recon_timeout =FALSE;
		   printf("Joe:poweron_recon_timeout_into_pair_disable\n");
		   break;
		}
		...
		if((bt_get_total_connect_dev() == 0) && !app_var.goto_poweroff_flag)
		{
			if(tws_api_get_tws_state() & TWS_STA_SIBLING_DISCONNECTED)
				...
				tws_play_tone_file(get_tone_files()->bt_mode, 400); //delay_msec>300以上，增加抗干扰情况
...
#if MMI_CC_DISCONNECTION_HCI_ENABLE
		case APP_MSG_DISCON_HCI:	
			printf("Joe:APP_MSG_DISCON_HCI\n");
			if(tws_api_get_role() == TWS_ROLE_SLAVE) {
				break;
			}
			...
			// test inquiry page scan func
			/*if(mmi_var.bt_discover_connectable==0x00)
			{
			   write_scan_conn_enable(0,0);//不可被发现，不能被连接
			}
			else if(mmi_var.bt_discover_connectable==0x01)
			{
			   write_scan_conn_enable(0,1);//不可被发现，可以被连接
			}
			else if(mmi_var.bt_discover_connectable==0x02)
			{
			   write_scan_conn_enable(1,0);//可以被发现，不能被连接
			}
			else if(mmi_var.bt_discover_connectable==0x03)
			{
			   write_scan_conn_enable(1,1);//可以被发现，可以被连接
			}
			else
			{
			   mmi_var.bt_discover_connectable =0x00;
			}*/
		break;
#endif
