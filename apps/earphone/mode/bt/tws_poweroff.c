...
{
    switch (msg[0]) {
#if MMI_CC_ENABLE_SHUT_DOWN_IN_CONNECTABLE
	case APP_MSG_BT_IN_PAGE_MODE:
	    if((mmi_var.page_type !=PAGE_TYPE_LINKLOSS) ||bt_get_total_connect_dev())
	       break;
#endif
...
#if MMI_CC_POWEROFF_BEFORE_WRITE_VM
    int ret =0x00;
	ret = syscfg_write(CFG_KEY_DOUBLE_CFG_ID, &double_cfg, sizeof(double_cfg_t));
	if(ret >0)
	{
	  log_info("Joe:Poweroff_write_config_success \n");
	}
	else
	{
	  log_info("Joe:Poweroff_write_config_fail\n");
	}
#endif
