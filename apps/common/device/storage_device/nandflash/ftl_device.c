...
{
    NAND_Result ret = nand_flash_page_read(block, page, offset, buf, len);

    if (ret == NAND_SUCCESS) {
        return FTL_ERR_NO;
    }
    if (ret == NAND_ECC_CORRECTED) {
...
{
    NAND_Result ret = nand_flash_page_write(block, page, 0, buf, len);
    if (ret == NAND_SUCCESS) {
        return FTL_ERR_NO;
    }
    if (ret == NAND_ECC_CORRECTED) {
...
static int ftl_nand_init()
{
    int ret = 0;

    if (g_capacity) {
        log_debug("ftl init succ");
        return 0;
    }
    ...
    u32 block_size = 1 << flash.block_size_shift;

    g_capacity = flash.logic_block_num << (flash.block_size_shift - 9);

    log_debug("ftl zone : %x -- size:%x", g_ftl_part_start_addr, g_capacity * 512);
    log_debug("ftl block_size : %x -- page size %x", block_size, 1 << flash.page_size_shift);
