...
#pragma bss_seg(".mpu6887p.data.bss")
#pragma data_seg(".mpu6887p.data")
#pragma const_seg(".mpu6887p.text.const")
#pragma code_seg(".mpu6887p.text")
#endif
...
unsigned char mpu6887p_set_fifo_wm_threshold(u16 watermark_level)//watermark_level:0~1023
{
    ...
    if (res == 1) {
        if ((temp_data & 0x80) != 0) {
            temp_data &= ~0x80;
            mpu6887p_write(MPU6887P_SLAVE_ADDRESS, Mpu6887pRegister_CONFIG, &temp_data, 1);
        }
    }
...
u8 mpu6887p_sensor_init(void *priv)
{
    ...
    spi_cs_init();
    spi_init(get_hw_spi_config(mpu6887p_info->spi_hdl));
#endif
//int module io init
