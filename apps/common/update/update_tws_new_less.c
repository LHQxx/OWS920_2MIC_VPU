...
static int tws_trans_data_write_callback(int ret)
{
    u8 rsp_data[3];
...
static void deal_sibling_tws_ota_trans(void *data, u16 len)
{
    ...
    case OTA_TWS_START_UPDATE:
        ...
        db_update_init(para.max_pkt_len);
        // 解除保护
        ...
        int ret = db_update_allow_check(para.fm_size);
    ...
    case OTA_TWS_TRANS_UPDATE_DATA:
        ...
        ret = db_update_write(recv_data + 2, len - 2, tws_trans_data_write_callback);
        if (ret < 0) {
            printf("tws update write err");
        }
    ...
    case OTA_TWS_VERIFY:
        ...
        ret = db_update_verify(tws_verify_result_hdl);
    ...
    case OTA_TWS_WRITE_BOOT_INFO:
        ...
        ret = db_update_burn_boot_info(0, 0, dual_bank_update_burn_boot_info_callback);
        if (ret < 0) {
            printf("set bootinfo err ,%d\n", ret);
        }
