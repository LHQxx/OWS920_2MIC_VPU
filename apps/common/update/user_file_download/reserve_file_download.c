...
#define SDFILE_EXT_RESERVED_ROOT_PATH		"flash/EXT_RESERVED/"
...
static int norflash_f_open(u32 file_len, u8 *file_name)
{
    ...
    u8 file_path_len = sizeof(SDFILE_EXT_RESERVED_ROOT_PATH) + strlen((char *)file_name);
    ...
    memcpy(file_path + strlen(SDFILE_APP_ROOT_PATH), file_name, strlen((const char *)file_name));
    fp = resfile_open((const char *)file_path);

    if (!fp) {
        memset(file_path, 0, file_path_len);
        memcpy(file_path, SDFILE_EXT_RESERVED_ROOT_PATH, strlen(SDFILE_EXT_RESERVED_ROOT_PATH));
        memcpy(file_path + strlen(SDFILE_EXT_RESERVED_ROOT_PATH), file_name, strlen((const char *)file_name));
        fp = resfile_open((const char *)file_path);
    }
    ...
    if (file_len > file_attr.fsize) {
        log_error("%s no enough to update\n", file_path);
...
static u32 reserve_file_sfc_write(const u8 *buf, u32 addr, u32 len)
{
    ...
    if (addr < __this->info.dev_addr || \
        addr + len > __this->info.dev_addr + __this->info.len) {
        log_error("overwrite\n");
        return 0;
    }
    ...
    flash_erase_addr_n_len_witout_align(addr, len, FLASH_SECTOR_SIZE);
...
static u32 reserve_file_sfc_erase(u32 cmd, u32 addr)
{
    return true;
}
        ...
        u32(**local_erase_hdl)(u32 cmd, u32 addr))
{
    ...
    if (local_read_hdl && local_write_hdl && local_erase_hdl) {
        log_info("reserved area start_addr %x and len %x update ...\n", reserve_info->info.dev_addr, reserve_info->info.len);
        __this = (reserve_file_update_info_t *)priv;
        *local_read_hdl = (u32(*)(void *, u32, u32))reserve_file_sfc_read;
        *local_write_hdl = (u32(*)(void *, u32, u32))reserve_file_sfc_write;
...
struct list_head *reserve_file_update_init(void *priv, u8 type, u8 *file_name, int (*update_info_get)(u8 type, user_chip_update_info_t *info, void *priv))
{
    ...
    memcpy(__this->info.name, file_name, sizeof(__this->info.name));
    __this->info.type = type;

    update_info_get(type, &__this->info, &__this->decode_func);
...
void get_remote_reserved_info_during_update_detail(u8 *reserved_name, u32 reserved_addr, u32 reserved_len, u8 opt)
{
    ...
    if (list_empty(&g_record_reserved_file_info->entry)) {
        if (0 == memcmp((const void *)reserved_name, (const void *)g_record_reserved_file_info->reserved_name, strlen((const char *)reserved_name) + 1)) {
        ...
        list_for_each_entry(item, &g_record_reserved_file_info->entry, entry) {
            if (0 == memcmp((const char *)item->reserved_name, (const char *)reserved_name, strlen((const char *)reserved_name) + 1)) {
