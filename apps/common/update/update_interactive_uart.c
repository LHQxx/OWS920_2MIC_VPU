...
static int uart_f_notify_len(void *priv, u32 size)
{
    y_printf("\n >>>[test]:func = %s,line= %d\n", __FUNCTION__, __LINE__);
    uart_update_cmd(CMD_UART_UPDATE_UPDATE_LEN, (u8 *)&size, 4);
    os_time_dly(50); //等待500ms
    return 0;
}
...
const update_op_api_t update_interactive_uart_ch_op = {
    ...
    .f_seek  = uart_f_seek,
    .f_stop  = uart_f_stop,
    .notify_update_content_size = uart_f_notify_len,
};
...
static void update_process_run(void)
{
    ...
    while (1) {
        ...
        case CMD_UART_UPDATE_UPDATE_LEN:
            log_info("CMD_UART_UPDATE_UPDATE_LEN\n");
            if (__this->master_ctl) { //主机
                ...
                /* uart_send_packet(pbuf, 1); */
                uart_update_cmd(pbuf[0], NULL, 0);
                if (up_inter_p->type == DUAL_BANK_UPDATA) {
                    const update_op_api_t *p = (update_op_api_t *)up_inter_p->p_op_api;
                    if (p && p->notify_update_content_size) {
                        p->notify_update_content_size(NULL, __this->update_total_size);
                    }
                    os_time_dly(50); //等待500ms
                }
            }
...
static void update_interactive_task_kill_ucos(void)
{
    ...
    argv[1] = 1;
    argv[2] = 0;
    int ret_update = os_taskq_post_type("app_core", Q_CALLBACK, 3, argv);
    if (ret_update) {
        return;
    }
...
static void update_interactive_task(void *ptr)
{
    ...
    if (!p && up_inter_p->type == DUAL_BANK_UPDATA && __this->update_sta == UPDATE_STA_SUCC) {
        y_printf(">>>[test]:--------------\n");
        __this->update_sta = 0;
        ufw_file_op_close();
    }
