...
static struct dac_demo dac_demo_var = {0};


static void dac_demo_run(void *p)
{
    puts("=DAC Demo Run=\n");
    u8 *ptr;
    s16 *data_addr16;
    ...
    while (1) {
        ...
        if (dac_demo_var.pend) {
            os_sem_pend(&dac_demo_var.sem, 0);
        }
#endif
...
#if DAC_DEMO_TASK_ENABLE
    task_create(dac_demo_run, NULL, "aud_dac_demo");
    os_sem_create(&dac_demo_var.sem, 0);
#else
    dac_demo_run(NULL);
#endif
}
...
#if DAC_DEMO_TASK_ENABLE
    dac_demo_var.pend = 1;
    task_kill("aud_dac_demo");
    y_printf("aud_dac_demo task kill succ");
#endif
    // 停止并关闭 DAC
    audio_dac_stop(&dac_hdl);
    audio_dac_close(&dac_hdl);
    dac_demo_var.active = 0;
}

#if AUDIO_DEMO_LP_REG_ENABLE
static u8 dac_demo_idle_query()
{
    return dac_demo.active ? 0 : 1;
}
